...
This document is the **definitive interview question bank and technical defense master guide** for the AI Technical Interviewer platform. It contains **75 exhaustive interview questions** organized across 8 technical dimensions—ranging from **Fresher/Junior fundamentals** to **Mid-Level engineering depth**, **Senior architectural trade-offs**, and **Staff/Principal system design & scaling**.
...
4. 🔗 **Codebase Source Anchor**: Direct references to relevant source files in the project.

---

# Table of Contents
- [Category 1: React 19, DOM, UI Performance & Client State (Q1–Q10)](#category-1-react-19-dom-ui-performance--client-state)
- [Category 2: Web Audio API, C++ Audio Graph & DSP Engineering (Q11–Q20)](#category-2-web-audio-api-c-audio-graph--dsp-engineering)
- [Category 3: Audio Codecs, WebM EBML Patching & IndexedDB Storage (Q21–Q30)](#category-3-audio-codecs-webm-ebml-patching--indexeddb-storage)
- [Category 4: WebSockets, Network Protocols & Real-Time Gateway (Q31–Q40)](#category-4-websockets-network-protocols--real-time-gateway)
- [Category 5: Concurrency, Event Loop & Database Mechanics (Q41–Q50)](#category-5-concurrency-event-loop--database-mechanics)
- [Category 6: AI Prompt Engineering, Cadence & Evaluation Rubrics (Q51–Q60)](#category-6-ai-prompt-engineering-cadence--evaluation-rubrics)
- [Category 7: Security, Threat Modeling & Prompt Injection (Q61–Q68)](#category-7-security-threat-modeling--prompt-injection)
- [Category 8: Staff & Principal System Design, Scale & FinOps (Q69–Q75)](#category-8-staff--principal-system-design-scale--finops)
- [Chapter 9: Rapid-Fire Verbal Defense Matrix (The "30-Second Elevator Answers")](#chapter-9-rapid-fire-verbal-defense-matrix-the-30-second-elevator-answers)
...
### Q11 [Junior]: What is the Web Audio API, and why is it superior to HTML5 `<audio>` for voice AI?
...
### Q12 [Mid-Level]: Explain the browser autoplay policy and how `LiveAudioPlayer.warmUp()` unlocks audio drivers.
...
### Q13 [Mid-Level]: How does your linear interpolation resampling algorithm work from 48kHz to 16kHz?
...
### Q14 [Senior]: Explain the Nyquist-Shannon sampling theorem and the exact frequency mathematics of 16kHz speech.
...
### Q15 [Mid-Level]: How does 16-bit Little-Endian signed integer PCM quantization work mathematically?
...
### Q16 [Senior]: How does sample-accurate gapless audio scheduling work using `nextPlayTime` in `LiveAudioPlayer`?
...
### Q17 [Senior]: How does client-side barge-in interruption detect voice energy and flush audio buffers in sub-10ms?
...
### Q18 [Mid-Level]: Explain the call stack overflow bug in V8 when Base64-encoding large byte arrays and your 32KB chunking fix.
...
### Q19 [Senior]: How does the dual-track `MediaStreamAudioDestinationNode` mix candidate mic and AI audio with 0ms latency?
...
### Q20 [Mid-Level]: Why is the AI recording gain set to `0.95` while candidate microphone gain is `1.05`?
...
### Q21 [Senior]: What is Chromium's WebM `Infinity` duration bug (`crbug/642012`), and why does `MediaRecorder` cause it?
...
### Q22 [Senior]: Walk through the binary byte-level logic of `webmDurationPatcher.ts`.
...
### Q23 [Mid-Level]: How does cross-browser codec negotiation pick between WebM Opus and Safari MP4 AAC?
...
### Q24 [Mid-Level]: How does `audioStorage.ts` use IndexedDB to store audio recordings with zero cloud costs?
...
### Q25 [Senior]: Explain the IndexedDB 5-session LRU auto-eviction algorithm and 7-day TTL cleanup job.
...
### Q26 [Junior]: How do you handle browsers where IndexedDB is blocked (e.g. Incognito / Private mode)?
...
### Q27 [Senior]: Why store raw binary `Blob` instances in IndexedDB instead of Base64 strings?
...
### Q28 [Junior]: How does the browser export recordings to disk via `URL.createObjectURL` without server requests?
...
### Q29 [Mid-Level]: How do you unit test Web Audio and IndexedDB in headless environments like Bun or Node?
...
### Q30 [Mid-Level]: What are the memory leak risks of `URL.createObjectURL` and how do you prevent them?
...
### Q31 [Senior]: Compare WebSockets vs HTTP/2 Server-Sent Events (SSE) vs WebRTC for bidirectional audio streaming.
...
### Q32 [Senior]: Walk through the upstream WebSocket handshake with Google Gemini Live (`BidiGenerateContentSetup`).
...
### Q33 [Mid-Level]: How does the backend proxy audio frames between candidate WebSocket and Google Gemini Live?
...
### Q34 [Senior]: How does the 30-second server grace period handle transient Wi-Fi drops without resetting interview state?
...
### Q35 [Mid-Level]: How do WebSocket heartbeats (`ping`/`pong`) prevent proxy disconnects on Render and Cloudflare?
...
### Q36 [Junior]: What is the difference between clean WebSocket closure code `1000` and abnormal closure `1006`?
...
### Q37 [Mid-Level]: How does the sliding-window IP rate limiter work, and how does BYOK bypass rate limits?
...
### Q38 [Junior]: Why use Express 5 with Bun instead of Next.js API routes or pure Node.js?
...
### Q39 [Mid-Level]: How do reverse proxies (Nginx/Caddy) handle WebSocket connection upgrades and long timeout directives?
...
### Q40 [Senior]: How do you prevent memory leaks when managing hundreds of concurrent active WebSocket instances?
...
### Q41 [Senior]: In single-threaded Bun/Node, how does `dbWriteQueue` prevent PostgreSQL I/O from stuttering audio playback?
...
### Q42 [Senior]: Explain the JavaScript Event Loop: Call Stack vs Microtask Queue vs Macrotask Queue during live streaming.
...
### Q43 [Mid-Level]: Explain your Prisma schema design: Why UUID v4 instead of autoincrementing integer IDs?
...
### Q44 [Senior]: Why is a compound index placed on `@@index([interviewId, turnIndex])` on the `Message` table?
...
### Q45 [Junior]: Why is `onDelete: Cascade` enforced on the `Message` relationship?
...
### Q46 [Mid-Level]: How does Prisma connection pooling work with `@prisma/adapter-pg` and Neon PostgreSQL Serverless?
...
### Q47 [Senior]: How would you handle database connection pool exhaustion if 1,000 interviews started simultaneously?
...
### Q48 [Mid-Level]: What is the difference between optimistic locking and pessimistic locking in state machines?
...
### Q49 [Junior]: How is the interview lifecycle state machine enforced in the database?
...
### Q50 [Junior]: What is the difference between `prisma db push` and `prisma migrate deploy`?
...
# Category 6: AI Prompt Engineering, Cadence & Evaluation Rubrics

### Q51 [Senior]: What is the "Staff Engineer Alex" persona, and what are the 14 core conversational invariants?
...
### Q52 [Mid-Level]: Explain the 2-Sentence Turn Formula and why airtime governance ($<20%$ AI airtime) is crucial.
...
### Q53 [Senior]: How does the 3-layer depth drill systematically probe candidate engineering answers?
...
### Q54 [Mid-Level]: How does the prompt engine handle candidate thinking pauses (*"Hmm, give me a second"*) without interrupting?
...
### Q55 [Mid-Level]: How does phonetic speech-to-text normalization handle engineering mispronunciations (*"post grass"*, *"read us"*)?
...
### Q56 [Senior]: How does the multi-track generator dynamically adapt questions across 8 domains and 3 seniority tiers?
...
### Q57 [Senior]: Explain the Multi-Model Evaluation Engine and the 25-second AbortController timeout race with Flash-Lite.
...
### Q58 [Senior]: What is the Anti-Sycophancy Gate, and how does `technicalAccuracy < 4.5` cap hiring recommendations?
...
### Q59 [Senior]: How does Originator Attribution ensure candidates only receive credit for self-originated concepts?
...
### Q60 [Mid-Level]: How does Candidate Reverse Q&A Isolation prevent interviewer answers from polluting candidate scores?
...
# Category 7: Security, Threat Modeling & Prompt Injection

### Q61 [Senior]: How does XML context sandboxing (`<candidate_project_readme>`) neutralize indirect prompt injection attacks?
...
### Q62 [Senior]: How is Bring-Your-Own-Key (BYOK) secured against server-side secret leaks or logging exposure?
...
### Q63 [Junior]: How do you prevent malicious candidates from executing Cross-Site Scripting (XSS) via transcript injection?
...
### Q64 [Mid-Level]: How do you defend against automated DDoS bots attempting to drain Gemini API credits?
...
### Q65 [Junior]: What is CORS, and how is it configured in Express?
...
### Q66 [Junior]: How does Helmet middleware harden backend HTTP headers?
...
### Q67 [Senior]: What are the security risks of WebSockets compared to REST, and how are unauthorized upgrades blocked?
...
### Q68 [Senior]: How do you comply with candidate data privacy regulations (GDPR / CCPA) with zero-cloud audio recording?
...
# Category 8: Staff & Principal System Design, Scale & FinOps

### Q69 [Staff/Principal]: Design a global, multi-region architecture to scale this platform to 100,000 concurrent live interviews.
...
### Q70 [Staff/Principal]: How would you implement distributed WebSocket session routing across a Kubernetes cluster using Redis Pub/Sub?
...
### Q71 [Senior]: Calculate the exact networking, CPU, and memory requirements for 10,000 concurrent audio streams.
...
### Q72 [Staff/Principal]: How would you introduce WebRTC SFU/MCU architecture if multi-party panel interviews were required?
...
### Q73 [Staff/Principal]: FinOps Analysis: Compare the cloud infrastructure cost of this architecture vs a traditional Whisper+GPT-4+ElevenLabs stack.
...
### Q74 [Senior]: How would you handle regional cloud outages (e.g. Google Cloud AI region down)?
...
### Q75 [Staff/Principal]: As a Staff Engineer, how would you measure the success, accuracy, and reliability of this voice AI platform?
...
# Chapter 9: Rapid-Fire Verbal Defense Matrix (The "30-Second Elevator Answers")
